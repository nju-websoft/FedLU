...
def main(args):
    ...
    for i in range(args.client_num):
        print("#entities of client %i: %i"%(i,len(client_es[i])))
    ...
    for i in range(args.client_num):
        ...
        unlearn_sets.append(read_triple(os.path.join(args.local_file_dir,"unlearn"+str(i)+".txt"),entity2id,relation2id))
        unlearn_triples.extend(unlearn_sets[i])

    all_true_triples = list(set(train_triples+valid_triples+test_triples)-set(unlearn_triples))
    ...
    retrain_triples = list(set(train_triples)-set(unlearn_triples))
    print("#train: %i"%(len(train_triples)))
    print("#retrain: %i"%(len(retrain_triples)))
    print("#test: %i" %(len(test_triples)))
    print("#unlearn: %i"%(len(unlearn_triples)))
    print("#test&train: %i"%(len(list(set(train_triples)&set(test_triples)))))
    print("#unlearn&train: %i"%(len(list(set(train_triples)&set(unlearn_triples)))))
    print("#test&retrain: %i"%(len(list(set(retrain_triples)&set(test_triples)))))
    print("#unlearn&retrain: %i"%(len(list(set(retrain_triples)&set(unlearn_triples)))))
    retrain_dataloader_head = DataLoader(
        TrainDataset(retrain_triples,nentity,nrelation,args.negative_sample_size,"head-batch"),
        batch_size = args.batch_size,
        shuffle = True,
        num_workers = max(0,args.cpu_num//2),
        collate_fn = TrainDataset.collate_fn
    )
    retrain_dataloader_tail = DataLoader(
        TrainDataset(retrain_triples,nentity,nrelation,args.negative_sample_size,"tail-batch"),
        batch_size = args.batch_size,
        shuffle = True,
        num_workers=max(0,args.cpu_num//2),
        collate_fn = TrainDataset.collate_fn
    )
    retrain_iterator = list(chain.from_iterable(zip(retrain_dataloader_head,retrain_dataloader_tail)))
    ...
    validtest_dataloader_head = DataLoader(
        TestDataset_Partial(valid_triples,all_true_triples,client_es[0],nentity,nrelation,"head-batch",),
        batch_size=args.test_batch_size,
        num_workers=max(0,args.cpu_num//2),
        collate_fn=TestDataset.collate_fn,
    )
    validtest_dataloader_tail = DataLoader(
        TestDataset_Partial(valid_triples,all_true_triples,client_es[0],nentity,nrelation,"tail-batch",),
        batch_size=args.test_batch_size,
        num_workers=max(0,args.cpu_num//2),
        collate_fn=TestDataset.collate_fn,
    )
    validtest_dataset_list = [validtest_dataloader_head, validtest_dataloader_tail]

    test_dataset_lists = []
    unlearn_dataset_lists = []
    for i in range(args.client_num):
        ...
        unlearn_dataloader_head = DataLoader(
            TestDataset_Partial(unlearn_sets[i],all_true_triples,client_es[i],nentity,nrelation,"head-batch",),
            batch_size=args.test_batch_size,
            num_workers=max(0,args.cpu_num//2),
            collate_fn=TestDataset.collate_fn,
        )
        unlearn_dataloader_tail = DataLoader(
            TestDataset_Partial(unlearn_sets[i],all_true_triples,client_es[i],nentity,nrelation,"tail-batch",),
            batch_size=args.test_batch_size,
            num_workers=max(0,args.cpu_num//2),
            collate_fn=TestDataset.collate_fn,
        )
        unlearn_dataset_lists.append([unlearn_dataloader_head,unlearn_dataloader_tail])
    optimizer = torch.optim.Adam(
        [{"params":kge_model.entity_embedding},{"params":kge_model.relation_embedding}],
    ...
    for epoch in range(0,args.max_epoch):
        for positive_sample,negative_sample,subsampling_weight,mode in retrain_iterator:
            log = kge_model.train_step(kge_model,optimizer,positive_sample,negative_sample,subsampling_weight,mode,args)
    ...
    metrics = kge_model.test_step(kge_model,validtest_dataset_list,args)
    log_metrics("evaluate on validtestset after training",best_epoch,metrics)

    unlearn_test_logs = []
    for i in range(args.client_num):
        metrics = kge_model.test_step(kge_model,unlearn_dataset_lists[i],args)
        unlearn_test_logs.append(metrics)
    print("Weighted Averaged and Deviation of Unlearn on all Clients")
    for metric in unlearn_test_logs[0].keys():
        if metric!="n":
            weighted = sum([unlearn_test_log[metric]*unlearn_test_log["n"] for unlearn_test_log in unlearn_test_logs])/sum([unlearn_test_log["n"] for unlearn_test_log in unlearn_test_logs])
            deviation = sum([((unlearn_test_log[metric]-weighted)**2)*unlearn_test_log["n"] for unlearn_test_log in unlearn_test_logs])/sum([unlearn_test_log["n"] for unlearn_test_log in unlearn_test_logs])
            print("%s:%f,%f"%(metric,weighted,deviation))

    test_logs = []
    for i in range(args.client_num):
        metrics = kge_model.test_step(kge_model,test_dataset_lists[i],args)
        test_logs.append(metrics)
    print("Weighted Averaged and Deviation of Test on all Clients")
    for metric in test_logs[0].keys():
        if metric!="n":
            weighted = sum([test_log[metric]*test_log["n"] for test_log in test_logs])/sum([test_log["n"] for test_log in test_logs])
            deviation = sum([((test_log[metric]-weighted)**2)*test_log["n"] for test_log in test_logs])/sum([test_log["n"] for test_log in test_logs])
            print("%s:%f,%f"%(metric,weighted,deviation))
